...
public class PacketSender {
    ...
    private static void sendPayload(@NotNull final Player receiver, String channel, ByteBuf bytes) {
        PacketPlayOutCustomPayload customPayload = new PacketPlayOutCustomPayload(new MinecraftKey(channel).toString(), new PacketDataSerializer(bytes));
    ...
    private static void sendPacket(@NotNull final CraftPlayer player, @NotNull final Packet packet) {
        Schedulers.async().run(() -> player.getHandle().playerConnection.sendPacket(packet));
    }
}